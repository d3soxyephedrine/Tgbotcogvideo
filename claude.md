# CogVideoX Setup Instructions for Novita GPU Server

**Purpose**: This document contains complete step-by-step instructions to set up the CogVideoX-5B video generation API on your Novita GPU server. Follow these instructions sequentially to deploy a production-ready video generation service.

---

## Overview

You will be deploying a FastAPI service that:
- Runs CogVideoX-5B model for text-to-video generation
- Exposes a REST API on port 8080
- Runs as a systemd service for automatic restarts
- Generates videos from text prompts in ~30-60 seconds per video

**Architecture**:
- FastAPI server with CogVideoX-5B pipeline
