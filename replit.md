...
- **Image Generation**: Flux 1.0 via Novita AI API for uncensored text-to-image capabilities using the `/imagine` command. Flux 1.0 is a 12B parameter model with exceptional quality and minimal content restrictions.
...
- **Novita AI (Flux 1.0)**: Uncensored image generation provider via async task-based API. Generates 1024×1024 images with ~$0.0015 cost per image.
