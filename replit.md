...
- **Image Generation**: FLUX.1 Kontext Max via Novita AI API for uncensored text-to-image capabilities using the `/imagine` command. FLUX.1 Kontext Max provides superior quality, exceptional prompt adherence, advanced typography capabilities, and truly uncensored 1024×1024 images. Uses safety_tolerance: "6" (maximum uncensored setting - range 0-6) for complete creative freedom with no content restrictions.
- **Image Editing**: FLUX.1 Kontext Max via Novita AI API for uncensored image-to-image editing. Users send a photo with a caption containing editing instructions. The bot downloads the photo from Telegram, passes it to Novita AI with the `images` parameter, and returns the edited result. Uses safety_tolerance: "6" but note that Novita AI automatically caps this at "2" when input images are provided (platform limitation) (10 credits per edit).
